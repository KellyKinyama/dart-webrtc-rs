...
class Alert {
  ...
  Future<void> marshal(ByteSink writer) async {
    await writer
        .add(Uint8List.fromList([alertLevel.value, alertDescription.value]));
    await writer.flush();
  }

  static Future<Alert> unmarshal(ByteReader reader) async {
    final alertLevelValue = await reader.readByte();
    final alertDescriptionValue = await reader.readByte();
    return Alert(AlertLevel.from(alertLevelValue),
        AlertDescription.from(alertDescriptionValue));
  }
